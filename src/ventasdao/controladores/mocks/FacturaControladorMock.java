...
public class FacturaControladorMock implements ICrud<Factura> {
    ...
    public FacturaControladorMock() {
        try {
            ...
            lineas1.add(new LineaFactura(1, 2.0, productos.get(0), 1)); // Leche
            lineas1.add(new LineaFactura(2, 1.0, productos.get(1), 1)); // Queso
            facturas.add(new Factura(1, new Date(), lineas1, TipoDePagoDeFactura.CONTADO));

            // Factura 2
            List<LineaFactura> lineas2 = new ArrayList<>();
            lineas2.add(new LineaFactura(3, 5.0, productos.get(2), 2)); // Pan
            facturas.add(new Factura(2, new Date(), lineas2, TipoDePagoDeFactura.DEBITO));

            // Factura 3
            List<LineaFactura> lineas3 = new ArrayList<>();
            lineas3.add(new LineaFactura(4, 3.0, productos.get(3), 3)); // Gaseosa
            lineas3.add(new LineaFactura(5, 2.0, productos.get(4), 3)); // Lavandina
            lineas3.add(new LineaFactura(6, 1.5, productos.get(5), 3)); // Asado
            facturas.add(new Factura(3, new Date(), lineas3, TipoDePagoDeFactura.TARJETA));

            // Factura 4
            List<LineaFactura> lineas4 = new ArrayList<>();
            lineas4.add(new LineaFactura(7, 10.0, productos.get(6), 4)); // Yogurt
            lineas4.add(new LineaFactura(8, 4.0, productos.get(0), 4)); // Leche
            facturas.add(new Factura(4, new Date(), lineas4, TipoDePagoDeFactura.CONTADO));

            // Factura 5
            List<LineaFactura> lineas5 = new ArrayList<>();
            lineas5.add(new LineaFactura(9, 1.0, productos.get(1), 5)); // Queso
            lineas5.add(new LineaFactura(10, 2.0, productos.get(2), 5)); // Pan
            lineas5.add(new LineaFactura(11, 6.0, productos.get(3), 5)); // Gaseosa
